...
1. the subject polygon
2. the clipping polygon (clipper)
...
But there is a problem with this approach in that it can not handle **concave** polygons. A concave polygon is any polygon that has an interior angle that measures between 180 and 360 degrees. Luckily there's a simple algorithm that can handle any polygon be it convex or concave. 
...
To be honest, this is actually a detour cuz we don't need it in the final algorithm. I just wanted to show it off. What we do need though is a way to get the intersection point of two lines.
...
I kinda want to explore one more [polygon clipping algorithm](https://en.wikipedia.org/wiki/Weiler%E2%80%93Atherton_clipping_algorithm) as the sutherland-hodgeman algo does have some limitations. For now, I think I'll probably switch to a different topic and come back to it in the future.
